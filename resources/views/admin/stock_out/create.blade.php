    ...
    <div style="display:flex; gap:15px;">
        ...
        <div style="flex:2">
            <div class="report-box">
                ...
                <div
                    style="background: #f8f9fa; padding: 8px; border-radius: 6px; border: 1px solid #e9ecef; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                    <button class="add-btn" onclick="addSelected()"
                        style="padding: 5px 12px; font-size: 11px; font-weight: 600; background-color: #007bff; border: none; border-radius: 4px; color: white; cursor: pointer; white-space: nowrap;">
                        ➕ Add Selected to Invoice
                    </button>
                    <div style="display: flex; align-items: center; gap: 6px; flex: 1;">
                        <label style="font-size: 11px; font-weight: 700; color: #495057; white-space: nowrap;">Customer
                            Selection:</label>
                        <select id="customerSelect" onchange="updateCustomerInfo(this)"
                            style="flex: 1; padding: 4px 8px; border-radius: 4px; border: 1px solid #ced4da; font-size: 11px; color: #495057; background-color: #fff;">
                            <option value="">-- Choose Customer --</option>
                            @foreach($customers as $c)
                                <option value="{{ $c->id }}" data-name="{{ $c->name }}" data-email="{{ $c->email }}"
                                    data-phone="{{ $c->phone }}" data-address="{{ $c->address }}">
                                    {{ $c->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($products as $p)
                            <tr>
                                <td><input type="checkbox" class="pcheck" data-id="{{ $p->id }}" data-name="{{ $p->name }}"
                                        data-price="{{ $p->unit_price }}"></td>
                                <td>{{ $p->product_code }}</td>
                                <td>{{ $p->name }}</td>
                                <td>{{ number_format($p->unit_price) }} FRW</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="left-fixed" style="flex:1">
            ...
            <hr>
            <p>Subtotal: <strong id="sub">0</strong> FRW</p>
            <p>Tax (18%): <strong id="tax">0</strong> FRW</p>
            <p><strong>Grand Total: <span id="grand">0</span> FRW</strong></p>

            <button class="generate-btn" style="background:#007bff" onclick="openPayment()">Process Payment & Create
                Invoice</button>
        </div>
    </div>
</div>
...
<div class="modal" id="payModal">
    <div class="modal-content" style="width: 480px;">
        ...
        <p style="background: #f8faff; padding: 10px; border-radius: 6px; border-left: 4px solid #007bff;">Amount Due:
            <strong id="payAmount" style="font-size: 18px; color: #e63946;"></strong> FRW</p>
        ...
        <input id="payInput" placeholder="Reference (Optional)"
            style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 6px; margin-bottom: 15px;">
        <button class="generate-btn" style="width: 100%; padding: 12px; font-weight: 700; background: #28a745;"
            onclick="confirmPayment()">Save Invoice & Get PDF</button>

        <div class="success-msg" id="paySuccess"
            style="margin-top: 15px; padding: 12px; text-align: center; border-radius: 6px; background: #d4edda; color: #155724; display: none;">
