...
<div class="page-center-container">
    <div class="crm-table-container">
        ...
        <form action="{{ route('admin.appointments.update', $appointment->id) }}" method="POST">
            ...
            <select id="user_id" name="user_id" onchange="updateCustomerInfo()">
                <option value="">-- No Customer --</option>
                @foreach($customers as $customer)
                    <option value="{{ $customer->id }}" {{ ($appointment->user_id == $customer->id) ? 'selected' : '' }}>
                        {{ $customer->name }}
                    </option>
                @endforeach
            </select>
            ...
            <div id="customer-info-box"
                style="background: rgba(79, 70, 229, 0.05); padding: 15px; border-radius: 8px; border: 1px dashed var(--primary); margin-bottom: 20px; display: none;">
                <div style="font-weight: bold; color: var(--primary); margin-bottom: 5px;">Customer Details</div>
                <div style="font-size: 13px; color: var(--text-primary);">
                    <div style="margin-bottom: 4px;"><i class="fas fa-envelope"
                            style="width: 20px; text-align: center; color: var(--text-muted);"></i> <span
                            id="info-email"></span></div>
                    <div style="margin-bottom: 4px;"><i class="fas fa-phone"
                            style="width: 20px; text-align: center; color: var(--text-muted);"></i> <span
                            id="info-phone"></span></div>
                    <div><i class="fas fa-map-marker-alt"
                            style="width: 20px; text-align: center; color: var(--text-muted);"></i> <span
                            id="info-address"></span></div>
                </div>
            </div>
            ...
            <script>
                ...
                function updateCustomerInfo() {
                    ...
                    const customer = customersData.find(c => c.id == userId);
                    if (customer) {
                        document.getElementById('info-email').textContent = customer.email || 'N/A';
                        document.getElementById('info-phone').textContent = customer.phone || 'N/A';
                        document.getElementById('info-address').textContent = customer.address || 'N/A';
