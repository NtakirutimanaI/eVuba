...
<body>
            ...
            @forelse($products as $product)
                <tr>
                    ...
                    <td style="color: #64748b; font-size: 10px;">
                        {{ $product->description ?? 'No specific technical notes registered.' }}</td>
                    <td>{{ $product->created_at->format('M d, Y') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="5" style="text-align: center; padding: 40px; color: #94a3b8;">No product entities found in
                        registry.</td>
                </tr>
            @endforelse
        </tbody>
    ...
    <div class="footer">
        This document is an official inventory record. Generated via eVuba Intelligence Dashboard. &copy;
        {{ date('Y') }} eVuba.
    </div>
</body>
