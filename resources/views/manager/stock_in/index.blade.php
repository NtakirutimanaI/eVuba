...
<div class="dashboard-wrapper">
    ...
    <div class="pro-header">
        <div>
            ...
            <p style="color: var(--secondary); margin-top: 0.5rem;">Orchestrate procurement and incoming supply chains
            </p>
        </div>
        <div class="header-actions" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
    ...
    <div class="main-grid">
        ...
        <div class="catalog-section">
            <div class="pro-card" style="padding: 0;">
                <div
                    style="padding: 1.5rem; border-bottom: 1px solid var(--glass-border); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                    <div class="pro-search" style="flex: 1; min-width: 250px; max-width: 400px;">
                        <i class="fas fa-search"></i>
                        <input type="text" id="internalSearch" placeholder="Search the ledger..."
                            style="background: none; border: none; width: 100%; focus: outline-none;">
                    </div>
                    <div style="display: flex; gap: 0.5rem;">
                        <button onclick="openFormModal('stockInModal')" class="action-btn btn-primary"
                            style="height: 38px;">
                    ...
                    <table class="pro-table" id="procurementLedger">
                        ...
                        <tbody>
                            @forelse($stockIns as $stock)
                                <tr>
                                    <td><span style="color: var(--secondary); font-weight: 600;">#{{ $stock->id }}</span>
                                    </td>
                                    <td>
                                        <div style="font-weight: 700; color: var(--dark);">
                                            {{ $stock->product->name ?? 'Deleted SKU' }}</div>
                                        <div style="font-size: 0.75rem; color: var(--secondary);">ID:
                                            {{ $stock->product_id }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <div style="font-size: 0.75rem; color: var(--secondary);">Total:
                                            {{ number_format($stock->total_cost) }}</div>
                                    </td>
                                    <td>
                                        <span class="status-badge"
                                            style="background: rgba(99, 102, 241, 0.1); color: var(--primary);">{{ ucfirst($stock->type) }}</span>
                                    </td>
                                    <td>
                                        <div style="font-weight: 600;">{{ $stock->stock_in_date->format('M d, Y') }}</div>
                                        <div style="font-size: 0.75rem; color: var(--secondary);">By:
                                            {{ $stock->user->name ?? 'System' }}</div>
                                    </td>
                                    <td>
                                        <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
                                            <button onclick="editStock({{ $stock->id }})" class="action-btn"
                                                style="background: rgba(99, 102, 241, 0.1); color: var(--primary);">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <form action="{{ route('manager.stock_in.destroy', $stock->id) }}" method="POST"
                                                onsubmit="return confirm('Archive this procurement log?')">
                                                @csrf @method('DELETE')
        ...
        <div class="sidebar-section">
            <div class="pro-card" style="margin-bottom: 1.5rem;">
                <h3 style="margin-bottom: 1.5rem; font-size: 1rem; color: var(--dark);"><i class="fas fa-chart-line"
                        style="margin-right: 0.5rem; color: var(--primary);"></i> Supply Pulse</h3>
                <canvas id="pulseChart" height="200"></canvas>
            </div>

            <div class="pro-card">
                <h3 style="margin-bottom: 1.5rem; font-size: 1rem; color: var(--dark);"><i class="fas fa-user-plus"
                        style="margin-right: 0.5rem; color: var(--primary);"></i> Partnerships</h3>
                <p style="font-size: 0.85rem; color: var(--secondary); margin-bottom: 1.5rem;">Register new strategic
                    sourcing partners</p>
                <button onclick="openFormModal('supplierModal')" class="action-btn"
                    style="width: 100%; height: 45px; background: rgba(99, 102, 241, 0.1); color: var(--primary); font-weight: 700;">
