...
<div class="dashboard-wrapper">
    <div class="pro-header">
        ...
        <div class="header-actions" style="display: flex; gap: 1rem; align-items: flex-end;">
            <form action="{{ route('manager.bookings.index') }}" method="GET" style="display: flex; gap: 0.75rem; align-items: flex-end; background: var(--white); padding: 1rem; border-radius: 1rem; border: 1px solid var(--glass-border);">
                ...
                <a href="{{ route('manager.bookings.index') }}" class="action-btn" style="height: 38px; background: var(--bg-main); color: var(--secondary);">
            ...
            <div style="display: flex; gap: 0.5rem;">
                <a id="pdfReportBtn" 
                   data-base-url="{{ route('manager.bookings.generateReport') }}"
                   href="{{ route('manager.bookings.generateReport', ['start_date' => request('start_date'), 'end_date' => request('end_date')]) }}" 
                   class="action-btn" style="background: var(--white); color: var(--secondary); height: 38px; padding: 0 1rem; width: auto; gap: 0.5rem;">
                    <i class="fas fa-file-pdf"></i> PDF
                </a>
                <a id="excelReportBtn" 
                   data-base-url="{{ route('manager.bookings.generateReport') }}"
                   href="{{ route('manager.bookings.generateReport', ['type' => 'excel', 'start_date' => request('start_date'), 'end_date' => request('end_date')]) }}" 
                   class="action-btn" style="background: var(--white); color: var(--secondary); height: 38px; padding: 0 1rem; width: auto; gap: 0.5rem;">
                            ...
                            <td>
                                <div style="display: flex; gap: 0.4rem; justify-content: flex-end;">
                                    @if($booking->status !== 'approved' && $booking->status !== 'completed')
                                        <form action="{{ route('manager.bookings.updateStatus', ['booking' => $booking->id, 'status' => 'approved']) }}" method="POST">
                                            @csrf
                                    ...
                                    @if($booking->status !== 'completed')
                                        <form action="{{ route('manager.bookings.updateStatus', ['booking' => $booking->id, 'status' => 'completed']) }}" method="POST">
                                            @csrf
                                    ...
                                    <a href="{{ route('manager.bookings.reschedule', $booking->id) }}" class="action-btn" title="Reschedule" style="background: rgba(99, 102, 241, 0.1); color: var(--primary);">
                                        <i class="fas fa-clock"></i>
                                    </a>

                                    <form action="{{ route('manager.bookings.updateStatus', ['booking' => $booking->id, 'status' => 'rejected']) }}" method="POST">
                                        @csrf
                                    ...
                                    <form action="{{ route('manager.bookings.updateStatus', ['booking' => $booking->id, 'status' => 'cancelled']) }}" method="POST" onsubmit="return confirm('Cancel this booking?');">
                                        @csrf
...
<style>
    ...
    .status-approved { background: rgba(34, 197, 94, 0.1); color: var(--success); }
    .status-pending { background: rgba(245, 158, 11, 0.1); color: var(--warning); }
    .status-completed { background: rgba(14, 165, 233, 0.1); color: var(--info); }
    .status-cancelled, .status-rejected { background: rgba(239, 68, 68, 0.1); color: var(--danger); }
